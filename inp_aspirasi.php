...
if (isset($_POST['submit'])) {
    ...
    if ($stmt->execute()) {
        var_dump($stmt->insert_id); // Debug: cek ID yang baru saja dimasukkan
        $id_pelaporan = $stmt->insert_id; // Ambil ID pelaporan yang baru saja dimasukkan
        $stmt1 = $koneksi->prepare("INSERT INTO aspirasi (id_pelaporan, status, id_kategori, feedback, tanggal) values (?, ?, ? ,? ,?)");
        $stmt1->bind_param("isiss", $id_pelaporan, $status, $id_kategori, $feedback, $tanggal);
        if ($stmt1->execute()) {
            echo "<script>
